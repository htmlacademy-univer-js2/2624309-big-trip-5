...
export default class EditPointView extends AbstractStatefulView {
  ...
  get template() {
    ...
    const typesMarkup = types.map((offerType) => `
      <div class="event__type-item">
        ...
        >
        <label
          class="event__type-label event__type-label--${offerType}"
          for="event-type-${offerType}"
        >
    ...
    // Пункты назначения
    const destinationsMarkup = this._destinations
    ...
    // Описание и фото пункта назначения
    const destinationDescription = destinationData.description
      ...
      : '';
    const photosMarkup = (destinationData.pictures || [])
  ...
  _restoreHandlers() {
    ...
    this._flatpickrStart = flatpickr(this.element.querySelector('#event-start-time-1'), {
      enableTime: true,
      dateFormat: 'd/m/y H:i',
      defaultDate: this._state.dateFrom,
      onChange: this._onStartDateChange.bind(this),
    });
    this._flatpickrEnd = flatpickr(this.element.querySelector('#event-end-time-1'), {
      enableTime: true,
      dateFormat: 'd/m/y H:i',
    ...
    // Обработчики динамики шаблона
    this.element.querySelector('.event__type-toggle')
      .addEventListener('click', this._toggleTypeList.bind(this));

    this.element.querySelectorAll('.event__type-input')
      .forEach((input) => input.addEventListener('change', this._onTypeChange.bind(this)));

    this.element.querySelector('.event__input--destination')
      .addEventListener('change', this._onDestinationChange.bind(this));
  ...
  _onStartDateChange([selectedDate]) {
    this.updateElement({ dateFrom: selectedDate });
  }

  _onEndDateChange([selectedDate]) {
